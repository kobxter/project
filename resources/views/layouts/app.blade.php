...
<head>
    ...
    <link rel="shortcut icon" type="x-icon" href="{{ asset('img/minilogo.png') }}">
...
<div class="sidebar">
    ...
    <a href="{{ url('/') }}">
        <img id="sidebar-logo" src="{{ asset('img/lo2.jpg') }}" alt="Logo" class="sidebar-logo">
    </a>
        ...
        @auth
            <li>
                <a href="{{ url('/') }}" class="{{ Request::is('/') ? 'active' : '' }}">
                    <i class="fa fa-calendar"></i> ปฏิทินการประชุม
                </a>
            </li>
            <!-- เพิ่มเงื่อนไขสำหรับ admin เท่านั้น -->
            @if (auth()->user()->role === 'admin')
                <li>
                    <a href="{{ route('users.index') }}" class="{{ Request::is('users*') ? 'active' : '' }}">
                        <i class="fa fa-users-cog"></i> จัดการผู้ใช้
                    </a>
                </li>
            @endif
            @if (auth()->user()->role === 'admin' || auth()->user()->role === 'staff')
                <li>
                    <a href="{{ url('/staffwork') }}" class="{{ Request::is('staffwork') ? 'active' : '' }}">
                        <i class="fa fa-briefcase"></i> ระบบจัดการประชุม
                    </a>
                </li>
            @endif
            <li>
            ...
            <li>
                <form id="logout-form" action="{{ route('logout') }}" method="POST" style="display:inline;">
                    @csrf
                    <button type="submit" class="btn btn-link text-white pl-0">
...
<script>
    $(document).ready(function () {
        $('.cl-toggle, #sidebar-collapse').on('click', function () {
            ...
            if (isCollapsed) {
                logo.attr('src', '{{ asset('img/lo2.jpg') }}');
